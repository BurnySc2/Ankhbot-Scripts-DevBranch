...
def Execute(data):
	...
	if data.IsChatMessage():
		if (data.GetParam(0).lower() == settings["command"] and Parent.HasPermission(data.User, settings["permission"], "")):
			tempResponseString = ""
			user = data.User			
			cd = ""

			if settings["costs"] > Parent.GetPoints(data.User):
				...
				if settings["useCooldown"] and (Parent.IsOnCooldown(ScriptName, settings["command"]) or Parent.IsOnUserCooldown(ScriptName, settings["command"], user)):
					...
					if slots.count(slot1) == 3:
						...
						Parent.AddPoints(user, int(settings["rewardTwoSeperated"]))
